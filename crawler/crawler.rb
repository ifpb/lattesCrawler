...
def extract_lattes(programResearchersInfo, researchers)
	threadsLattesPool = Thread.pool(150)
...
def extract_page(programsIds, programsInfo, researchers)
		...
		threadsResearchPool.process do
			...
			if idProgram == '31001017004P3' #UFRJ multiple pages
				programInfo['researchers'].each{ |page|
					doc = open_page(page, '_')
					programResearchersInfo[idProgram].concat extract_researchers_info(idProgram, doc, programInfo)
				}
			else
				page = programInfo['researchers']
				doc = open_page(page, '_')
				programResearchersInfo[idProgram] = extract_researchers_info(idProgram, doc, programInfo)
			end
			# puts "================="
...
programResearchersInfo1 = extract_page(programsIds1, programsInfo, researchers)
puts
extract_lattes(programResearchersInfo1, researchers)
puts
#UFMS
OpenSSL::SSL::VERIFY_PEER = OpenSSL::SSL::VERIFY_NONE
programResearchersInfo2 = extract_page(programsIds2, programsInfo, researchers)
puts
...
generate_stat(programsInfo, researchers)
